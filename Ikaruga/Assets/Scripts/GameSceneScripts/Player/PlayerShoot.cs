...
public class PlayerShoot : MonoBehaviour
{
    // PUBLIC
        ...
        private bool alreadyLooping = true;
    ...
    void Update()
    {
        if (PauseMenu.isPaused && alreadyLooping)
        {
            audioManager.Loop("Player_BulletShot");
            alreadyLooping = false; // tell the game that the shooting is already paused, so don't worry about it
        }
        else if (!PauseMenu.isPaused && !alreadyLooping)
        {
            audioManager.Loop("Player_BulletShot");
            alreadyLooping = true; // tell the game that we already resumed the loop, so don't worry about it
        }
